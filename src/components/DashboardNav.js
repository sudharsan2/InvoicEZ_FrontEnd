...
const DashboardNav = () => {
  ...
  const counters = [
    {
      label: "Dashboard",
      value: <span style={{ color: "#004378" }}>PO Calender</span>,
      color: "#004378",
    },
  ...
  const counter2 = [
    {
      label: "Dashboard",
      value: <span style={{ color: "#004378" }}>Live Count</span>,
      color: "#004378",
    }, // Cyan
  ];
  ...
  const others = [
    {
      label: "Dashboard",
      value: <span style={{ color: "#004378" }}>Others</span>,
      color: "#004378",
    }, // Cyan
  ];
  ...
  const TodoCount = [
    {
      label: "Dashboard",
      value: <span style={{ color: "#004378" }}>To Do</span>,
      color: "#004392",
    },
    {
      label: "Yet To Process",
      value: <span style={{ color: "#004378" }}>{gatelength}</span>,
      color: "#ff0000",
    },
    {
      label: "Waiting For GRN",
      value: <span style={{ color: "#004378" }}>{tableLength}</span>,
      color: "#b80da3",
    }, // Cyan
  ];
  ...
  const OpenPo = async (showMessage = false) => {
    ...
    try {
      ...
      const getUniqueOptions = (array, key) => {
        return Array.from(new Set(array.map((item) => item[key]))).map(
          (value) => ({
            value,
            label: value,
          }),
  ...
  const HistoryData = async (showMessage = false) => {
    ...
    try {
      ...
      const fetchedItems = response.data;
      console.log("fetchedItemsNAV", fetchedItems);
      ...
      const mappedItems = fetchedItems.map((item, index) => {
        return {
          Id:
            item.invoiceInfo.po_headers &&
            item.invoiceInfo.po_headers.length > 0
              ? item.po_headers[0].id
              : null,
          grn_num: item.invoiceInfo.gate_entry_no,
          location:
            item.invoiceInfo.po_headers &&
            item.invoiceInfo.po_headers.length > 0
              ? item.invoiceInfo.po_headers[0].ship_to
              : null,
          po_number:
            item.invoiceInfo.po_headers &&
            item.invoiceInfo.po_headers.length > 0
              ? item.invoiceInfo.po_headers[0].po_number
              : null,
          received_date: item.invoiceInfo.receivedDate,
          supplier_name: item.invoiceInfo.VendorName,
          total_amount: item.invoiceInfo.InvoiceTotal,
          receipt: item.invoiceInfo.receipt_number,
